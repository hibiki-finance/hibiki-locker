...
contract LockerTest is Test {
    ...
    function test_RevertWhen_NoFeePaid() public {
        vm.expectRevert(abi.encodeWithSelector(HibikiLocker.WrongFee.selector, 0, locker.gasFee()));
    ...
    function test_LockTokens() public {
        ...
        locker.lock{value: locker.gasFee()}(address(erc20t), lockAmount, uint32(block.timestamp + 60));
    ...
    function test_RevertWhen_UnlockBeforeTime() public {
        locker.lock{value: locker.gasFee()}(address(erc20t), 1 ether, uint32(block.timestamp + 200));
